...
assert agent["command"] == "/lab/agents/claude-code/trust-workspace.sh"
assert agent["args"][:2] == ["--exec", "claude"]
assert agent["args"][2:] == ["--model", "claude-opus-4-8", "--dangerously-skip-permissions"]
...
PY
then
  _pass "all Gastown roles use the existing hook in exec mode"
else
  _fail "all Gastown roles use the existing hook in exec mode"
fi
