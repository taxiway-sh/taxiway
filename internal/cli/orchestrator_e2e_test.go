...
func testE2EOrchestratorPrepareRun(t *testing.T, orch string) {
	...
	runE2EStep(t, e2eCommandStepAt("after-start", "shell", "--check"), func(t *testing.T) {
		...
		assertE2EOrchestratorSessions(t, state, id, orch)
	})
...
func assertE2EAgentWorkspaceTrusted(t *testing.T, state *RootState, id, orch, workspacePath string) {
	...
	case "claude-code":
		argv = []string{
			...
			"/home/taxiway/.claude.json",
		}
	case "codex":
		argv = []string{
			"python3", "-c",
			`import sys, tomllib; config = tomllib.load(open(sys.argv[1], "rb")); assert config["projects"][sys.argv[2]]["trust_level"] == "trusted"`,
